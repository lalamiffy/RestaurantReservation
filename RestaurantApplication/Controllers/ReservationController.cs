...
namespace RestaurantApplication.Controllers
{
    public class ReservationController : Controller
    {
        ...
        {    
            ...
            //int vacancies = sitting.Capacity - sitting.Main.Sum(r => r.Guests); 
            if(sitting == null)
            {
                return BadRequest(); 
            }
            ...
            if (sitting != null)
            {
                m.DateTimeRange1 = sitting.StartTime;
                m.DateTimeRange2 = sitting.EndTime;
                m.Sitting = $"{sitting.SittingType.Name} {sitting.StartTime:yyyy-MM-dd} {sitting.StartTime:hh:mm tt} - {sitting.EndTime:hh:mm tt}";

                if (m.StartTime == default)
                {
                    m.StartTime = sitting.StartTime;
                }
            }

            m.ReservationSources = new SelectList(_context.ReservationSources, "Id", "Name", m.ReservationSourceId); 
            return View(m); 
        }
